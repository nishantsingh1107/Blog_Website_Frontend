...
const ProfilePage = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const resp = await axiosInstance.get("/users/", { withCredentials: true });
    ...
    const handlePhotoChange = async (e) => {
        ...
        if (file) {
            ...
            try {
                const resp = await axiosInstance.post("/users/profile-photo", formData, {
            ...
            <div className="flex flex-col items-center justify-center mt-10 px-4 pb-16">
                <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8 border border-blue-100 flex flex-col items-center">
                    <div className="relative mb-6">
                        ...
                        <button
                            className="absolute bottom-2 right-2 bg-blue-600 text-white rounded-full p-2 shadow hover:bg-blue-700 focus:outline-none disabled:opacity-50"
                    ...
                    {editMode ? (
                        ...
                        <>
                            <h2 className="text-2xl font-bold text-blue-700 mb-2">{user?.name || "No Name"}</h2>
                            <p className="text-gray-600 mb-1">{user?.email}</p>
                            <p className="text-gray-400 text-sm mb-4">{user?.gender ? user.gender.charAt(0).toUpperCase() + user.gender.slice(1) : ""}</p>
                            <button
